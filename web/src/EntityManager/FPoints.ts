...
export class FPoints extends FEntityRecord {
    constructor() {
        super();
    }
    ...
    create(): void {
        ...
        //创建对象
        this.entity = new THREE.Points(this.m_bufferGeometry, material.material());
    }
    ...
    updateVariants(_vertexs: Float64Array, _len: number): void {
        ...
        (<THREE.Points>this.entity).geometry.attributes.position.needsUpdate = true;
    }
    ...
    setPosition(_position: Ge.GePoint3d): void {
        (<THREE.Points>this.entity).position.x = _position.x;
        (<THREE.Points>this.entity).position.y = _position.y;
        (<THREE.Points>this.entity).position.z = _position.z;
    }

    // 获得缩放
    scale(): Ge.GeScale3d {
        return new Ge.GeScale3d((<THREE.Points>this.entity).scale.x, (<THREE.Points>this.entity).scale.y, (<THREE.Points>this.entity).scale.z);
    }

    // 设置缩放
    setScale(_scale: Ge.GeScale3d): void {
        (<THREE.Points>this.entity).scale.x = _scale.sx;
        (<THREE.Points>this.entity).scale.y = _scale.sy;
        (<THREE.Points>this.entity).scale.z = 1;
    }

    // 设置轴角
    setRotateOnAxis(_axis: Ge.GeVector3d, _angle: number): void {
        (<THREE.Points>this.entity).rotateOnAxis(new THREE.Vector3(_axis.x, _axis.y, _axis.z), _angle);
    }
