...
export class FInterface {
    ...
    /** 格式化命令参数 */
    static parseParams(_Param: string): Object {
        ...
        do {
            ...
            for (let i = 0; i < _Param.length; i++) {
                ...
                if (IsName == true && _Param[i] == '=') {
                    IsName = false;
                    IsValue = true;
                    continue;
                }
                if (IsValue == true && (_Param[i] == '&' || i == _Param.length - 1)) {
                    IsName = true;
                    IsValue = false;
    ...
    /** 选择文件 */
    static async selectLocalFiles(_Accept: string = ""): Promise<FileList | null> {
        return new Promise((resolve) => {
            ...
            input.onchange = (eventevent: Event) => {

                let inputElement: HTMLInputElement = <HTMLInputElement>eventevent.target;
    ...
    /** 保存文件 */
    static saveFile(_content: Blob | string, _fileName: string): void {
        do {

            //获得Url
            let url: string = "";
            if (typeof _content === 'string') {
                url = URL.createObjectURL(new Blob([_content], { type: "text/plain;charset=utf-8" }))
            }
            else {
                url = URL.createObjectURL(_content)
            }
    ...
    /** 下载文件 */
    static download(_url: string, _fileName: string = ''): void {
    ...
    /** 读取服务器文件 */
    static async readServerFile(_Url: string): Promise<string> {
    ...
    /** 加载文件 */
    static async loadFileBuffer(_file: File): Promise<ArrayBuffer> {
        return new Promise((resolve) => {
            let reader = new FileReader();
            reader.readAsArrayBuffer(_file);
            reader.onload = function (_event: any) {
    ...
    /** 加载文本文件 */
    static async loadFileText(_file: File, _code: string = "UTF8"): Promise<string> {
        return new Promise((resolve) => {
            let reader = new FileReader();
            reader.readAsText(_file, _code);
            reader.onload = function (_event: any) {
                resolve(_event.target.result);
            }
        });
    ...
    /** 复制内容到剪切板 */
    static copyToClip(_Content: string): void {
    ...
    /** uint32颜色转rgb */
    static converArgbToRgb(argb: number): Array<number> {
        let rgb: Array<number> = new Array<number>();
        rgb.push((argb & 0xff0000) >> 16);
        rgb.push((argb & 0xff00) >> 8);
        rgb.push((argb & 0xff));
        return rgb;
    }

    /** colorIndex转rgb */
    static colorIndexToRGB = (_colorIndex: number): { r: number, g: number, b: number } => {
