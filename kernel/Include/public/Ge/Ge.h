...
#define radianToAngle(x) x*180.0/PI
#define angleToRadian(x) x/180.0*PI
#define RadianNormalize(ang) \
    ang = ang - (int(ang / (PI * 2)) * PI * 2);\

/** \details
  The Ge classes are used to carry out general 2D and 3D geometric operations. 
...
struct Ge
{
  static const GeLibVersion gLibVersion;
...
typedef Ge::ErrorCondition GeError;
