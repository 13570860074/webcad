...
int acutVSPrintf(char *buf, size_t nBufLen, const char *fmt, va_list va);

template<size_t nBufLen> inline int acutSPrintf(char (& buf)[nBufLen], const ACHAR *format, ...)
{
    va_list va;
    va_start(va, format);
...
template<size_t nBufLen> inline int acutVSPrintf(char (& buf)[nBufLen], const ACHAR *format,
                                                 va_list va)
{
